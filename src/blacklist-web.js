...
export function normalizeBlacklistDiscordIdentity(identity, userId) {
  const source = identity && typeof identity === 'object' ? identity : {};
  const nestedUser = source.user && typeof source.user === 'object' ? source.user : {};
  const resolvedId = normalizeDiscordUserId(source.id ?? nestedUser.id ?? userId);
  const username = cleanText(source.username ?? nestedUser.username, 100);
  const globalName = cleanText(source.globalName ?? source.global_name ?? nestedUser.globalName ?? nestedUser.global_name, 100);
  const displayName = cleanText(source.displayName ?? source.display_name ?? source.nick ?? nestedUser.displayName ?? nestedUser.globalName ?? nestedUser.global_name, 100);
  const discriminator = cleanText(source.discriminator ?? nestedUser.discriminator, 8);
  const tag = username && discriminator && discriminator !== '0' ? `${username}#${discriminator}` : username;
  const avatar = cleanText(source.avatar ?? nestedUser.avatar, 256);
  const existingAvatarUrl = cleanText(source.avatarUrl, 500);
  const avatarUrl = /^https:\/\/cdn\.discordapp\.com\/(?:avatars|embed\/avatars)\//i.test(existingAvatarUrl)
    ? existingAvatarUrl
    : buildDiscordAvatarUrl(resolvedId || userId, avatar);
  return {
    id: resolvedId,
    username,
    globalName,
    displayName: displayName || globalName || username,
    tag,
    avatarUrl,
    resolved: Boolean(username || globalName || displayName)
  };
}

function buildDiscordAvatarUrl(userId, avatar) {
  const id = normalizeDiscordUserId(userId);
  if (id && avatar && /^[-_a-zA-Z0-9]+$/.test(avatar)) {
    const extension = avatar.startsWith('a_') ? 'gif' : 'png';
    return `https://cdn.discordapp.com/avatars/${encodeURIComponent(id)}/${avatar}.${extension}?size=128`;
  }
  let index = 0;
  try {
    index = Number((BigInt(id || '0') >> 22n) % 6n);
  } catch {
    index = 0;
  }
...
export async function getBlacklistWebRecord({ storage, value, identity = null }) {
  ...
  return serializeBlacklistWebRecord({ entry, evidence, identity });
}

export function serializeBlacklistWebRecord({ entry, evidence = [], proofNote = '', identity = null }) {
  const userId = String(entry?.userId ?? '').trim();
  const discordIdentity = normalizeBlacklistDiscordIdentity(identity, userId);
...
function renderRecord(record, isOwner) {
  const { entry, identity = {}, evidence, proofNote = '', profileUrl, activeNow } = record;
  const status = activeNow ? 'Activa' : entry.active ? 'Caducada' : 'Inactiva';
  const statusClass = activeNow ? 'active' : 'inactive';
  const displayName = identity.displayName || identity.username || 'Usuario de Discord';
  const usernameLine = identity.username ? `@${identity.username}` : 'Identidad no disponible';
  ...
  return `<section class="result-card" id="result" aria-live="polite">
    ...
    <div class="identity-row"><img class="identity-avatar" src="${escapeHtml(identity.avatarUrl || buildDiscordAvatarUrl(entry.userId, ''))}" alt="Avatar de ${escapeHtml(displayName)}" loading="eager" referrerpolicy="no-referrer"><div class="identity-copy"><strong>${escapeHtml(displayName)}</strong><span>${escapeHtml(usernameLine)}</span><span class="mono">ID ${escapeHtml(entry.userId)}</span></div><a class="profile-link" href="${escapeHtml(profileUrl)}" target="_blank" rel="noopener noreferrer">Ver perfil ↗</a></div>
    <div class="info-grid">
...
function renderNoBlacklistRecord(query) {
  return `<section class="notice clear-result" id="result" aria-live="polite">
    <div class="clear-result-top"><span class="clear-icon" aria-hidden="true">✓</span><div><p class="eyebrow">Resultado de búsqueda</p><h2>Sin sanciones registradas</h2></div></div>
    <p>La consulta no encontró a este usuario en ninguna de las blacklists consultadas por NexaDesk.</p>
    <div class="clear-result-query"><span>Consulta realizada</span><strong>${escapeHtml(query)}</strong></div>
    <p>Si crees que hemos tenido un error o que tu usuario ha sido sancionado por equivocación, cuéntanoslo y revisaremos el caso.</p>
    <a class="button support-button" href="https://discord.gg/vVXbq7ePEZ" target="_blank" rel="noopener noreferrer">¿Hemos tenido un error? Dínoslo en nuestro servidor de soporte ↗</a>
  </section>`;
}
...
export function renderBlacklistPage({ query = '', record = null, error = '', isOwner = false } = {}) {
  ...
  const content = error
    ...
    : record
      ...
      : query
        ? renderNoBlacklistRecord(query)
        : '<section class="empty"><div class="empty-icon">⌕</div><p class="eyebrow">Consulta pública</p><h2>La información que necesitas, sin ruido.</h2><p>Busca por ID, username de Discord o código global para consultar el estado, la fecha, el origen, el motivo y las pruebas disponibles.</p></section>';
  return `<!doctype html>
<html lang="es">
<head>
