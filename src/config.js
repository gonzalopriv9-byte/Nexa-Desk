...
const schema = z.object({
  ...
  TURNSTILE_VERIFY_TIMEOUT_MS: z.coerce.number().int().min(1000).max(15000).default(5000),
  ...
  AI_PROVIDER: z.enum(['google-ai-studio', 'groq', 'ollama', 'openai-compatible', 'disabled']).default('google-ai-studio'),
  ...
  AI_PROVIDER_COOLDOWN_MS: z.coerce.number().int().min(5000).max(600000).default(60000),
  AI_SERVER_CONTEXT_TIMEOUT_MS: z.coerce.number().int().min(300).max(5000).default(900),
  ...
  GEMINI_TTS_TIMEOUT_MS: z.coerce.number().int().min(1000).max(30000).default(8000),
  GEMINI_TTS_MAX_STREAM_MS: z.coerce.number().int().min(5000).max(120000).default(45000),
  ...
  VOICE_STT_LANGUAGE: z.string().default('es'),
  VOICE_STT_PROMPT: z.string().default('NexaDesk, Nexa Desk, Nexa, Discord, ticket, canal de voz, staff, IA, /ticket voz crear.'),
  VOICE_STT_TIMEOUT_MS: z.coerce.number().int().min(1500).max(30000).default(7000),
  VOICE_STT_CLEANUP_ENABLED: envBoolean.default(false),
  ...
  FISH_AUDIO_TTS_REFERENCE_ID: z.string().optional().default('21adf3cda02a4aa88dc593353cc9d715'),
  FISH_AUDIO_TTS_FORMAT: z.enum(['opus', 'mp3', 'wav', 'pcm']).default('opus'),
  FISH_AUDIO_TTS_LATENCY: z.enum(['low', 'balanced', 'normal']).default('low'),
  FISH_AUDIO_TTS_STREAM_FORMAT: z.enum(['mp3', 'opus', 'pcm']).default('mp3'),
  FISH_AUDIO_TTS_STREAM_LATENCY: z.enum(['balanced', 'normal', 'low']).default('low'),
  FISH_AUDIO_TTS_STREAM_CHUNK_LENGTH: z.coerce.number().int().min(80).max(300).default(80),
  FISH_AUDIO_TTS_STREAM_MIN_CHUNK_LENGTH: z.coerce.number().int().min(20).max(160).default(25),
  FISH_AUDIO_TTS_STREAM_MP3_BITRATE: z.coerce.number().int().refine((value) => [64, 128, 192].includes(value), {
    message: 'FISH_AUDIO_TTS_STREAM_MP3_BITRATE must be 64, 128 or 192'
  }).default(64),
  ...
  FISH_AUDIO_TTS_SPEED: z.coerce.number().min(0.75).max(1.25).default(1.08),
  ...
  FISH_AUDIO_TTS_MAX_CHARS: z.coerce.number().int().min(120).max(500).default(260),
  FISH_AUDIO_TIMEOUT_MS: z.coerce.number().int().min(3000).max(30000).default(5000),
  ...
  BACKUPS_ENABLED: envBoolean.default(true),
  BACKUP_INTERVAL_MS: z.coerce.number().int().min(900000).max(86400000).default(3600000),
  BACKUP_STARTUP_DELAY_MS: z.coerce.number().int().min(5000).max(300000).default(30000),
