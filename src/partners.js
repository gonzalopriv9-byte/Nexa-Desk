...
function renderOwnerScript() {
  return `<script>
    (() => {
      ...
      function resetForm() { state.editingId = ''; state.pendingIcon = null; state.pendingMedia = null; form.reset(); byId('partnerId').value = ''; status(''); }
      ...
      form.addEventListener('submit', async (event) => { event.preventDefault(); const title = byId('partnerTitle').value.trim(); if (!title) return status('El título es obligatorio.', true); if (!state.editingId && state.partners.length >= ${MAX_PARTNERS}) return status('Has alcanzado el límite de partners.', true); const existing = state.partners.find((item) => item.id === state.editingId); const rawDiscord = byId('partnerDiscordUrl').value.trim(); if (rawDiscord) { try { const discordUrl = new URL(rawDiscord); const allowedHosts = ['discord.gg', 'discord.com', 'discordapp.com']; if (discordUrl.protocol !== 'https:' || !allowedHosts.includes(discordUrl.hostname.toLowerCase())) return status('El enlace debe ser de Discord.', true); } catch { return status('El enlace debe ser de Discord.', true); } } const partner = { ...(existing || {}), id: state.editingId || ('partner-' + Date.now()), title, bio:byId('partnerBio').value.trim(), discordUrl:rawDiscord, whatItDoes:byId('partnerWhatItDoes').value.trim(), iconUrl:state.pendingIcon?.url || existing?.iconUrl || '', media:state.pendingMedia || existing?.media || null }; if (state.editingId) state.partners = state.partners.map((item) => item.id === state.editingId ? partner : item); else state.partners.push(partner); try { status('Guardando...'); const response = await fetch('/partners/api', { method:'POST', credentials:'same-origin', headers:{'content-type':'application/json'}, body:JSON.stringify({ partners:state.partners }) }); const data = await response.json().catch(() => ({})); if (!response.ok) throw new Error(data.error || 'No se pudo guardar.'); state.partners = data.partners || state.partners; state.pendingIcon = null; state.pendingMedia = null; resetForm(); renderItems(); status('Partner publicado correctamente.'); setTimeout(() => window.location.reload(), 700); } catch (error) { status(error.message, true); } });
      byId('newPartnerButton').addEventListener('click', resetForm);
      document.addEventListener('click', async (event) => { const edit = event.target.closest('[data-edit-partner]'); if (edit) { loadPartner(edit.dataset.editPartner); return; } const remove = event.target.closest('[data-delete-partner]'); if (!remove) return; const partner = state.partners.find((item) => item.id === remove.dataset.deletePartner); if (!partner || !window.confirm('¿Borrar ' + partner.title + '?')) return; const previous = state.partners; state.partners = state.partners.filter((item) => item.id !== partner.id); renderItems(); status('Eliminando ' + partner.title + '...'); try { const response = await fetch('/partners/api', { method:'POST', credentials:'same-origin', headers:{'content-type':'application/json'}, body:JSON.stringify({ partners:state.partners }) }); const data = await response.json().catch(() => ({})); if (!response.ok) throw new Error(data.error || 'No se pudo eliminar el partner.'); state.partners = data.partners || state.partners; if (state.editingId === partner.id) resetForm(); renderItems(); status('Partner eliminado correctamente.'); setTimeout(() => window.location.reload(), 500); } catch (error) { state.partners = previous; renderItems(); status(error.message, true); } });
      renderItems();
    })();
  </script>`;
}
